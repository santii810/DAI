...
public abstract class MultipleServersTestCase {
	...
	private static final String[] WILDCARD_HTML_UUIDS = new String[] { "6df1047e-cf19-XXXX-8cf3-38f5e53f7725",
			"79e01232-5ea4-XXXX-9331-1c1880a1d3c2", "a35b6c5e-22d6-XXXX-98b4-462482e26c9e",
			"3aff2f9c-0c7f-XXXX-99ad-27a0cf1af137", "77ec1d68-84e1-XXXX-be8e-066e02f4e373" };

	private static final String[] WILDCARD_TRANSFORMED_FILES = new String[] { "files/sample1_X.html",
			"files/sample2_X.html" };

	private static final String[] WILDCARD_XML_FILES = new String[] { "files/sample1_X.xml", "files/sample2_X.xml" };

	private static final String[] WILDCARD_XML_UUIDS = new String[] { "ddcab7d0-636c-XXXX-8db3-685b35c84fb4",
			"ea118888-6908-XXXX-9620-685b35c84fb4" };

	private static final String[] WILDCARD_XSD_FILES = new String[] { "files/sample1_X.xsd", "files/sample2_X.xsd" };

	private static final String[] WILDCARD_XSD_UUIDS = new String[] { "e5b64c34-636c-XXXX-b729-685b35c84fb4",
			"05b88faa-6909-XXXX-aadc-685b35c84fb4" };

	private static final String[] WILDCARD_XSLT_FILES = new String[] { "files/sample1_X.xslt", "files/sample2_X.xslt" };

	private static final String[] WILDCARD_XSLT_UUIDS = new String[] { "f260dfee-636c-XXXX-bbdd-685b35c84fb4",
	...
	private static String[] getAllResources(String[] wildcardResources) {
		...
		for (int i = 0; i < NUMBER_OF_SERVERS; i++) {
			for (int j = 0; j < wildcardResources.length; j++) {
				uuids[index++] = wildcardResources[j].replaceAll("X", Integer.toString(i + 1));
			}
		}
	...
	protected static String[] getFileContents(String[] fileNames) {
		...
		for (int i = 0; i < fileNames.length; i++) {
			contents[i] = readToString(clazz.getResourceAsStream(fileNames[i]));
		}
	...
	private static String[] getLocalResources(int serverIndex, String[] wildcardResources) {
		...
		for (int i = 0; i < wildcardResources.length; i++) {
			uuids[i] = wildcardResources[i].replaceAll("X", Integer.toString(serverIndex + 1));
		}
	...
	protected JdbcDatabaseTester createDatabaseTester(int index) throws ClassNotFoundException {
		return new MySqlJdbcDatabaseTester(getConnectionUrl(index), getUsername(), getPassword());
	}

	protected IDataSet createDataSet(int index) throws Exception {
		return new FlatXmlDataSetBuilder().setMetaDataSetFromDtd(getClass().getResourceAsStream("files/dataset.dtd"))
	...
	public void setUpServers() throws Exception {
		...
		for (int i = 0; i < 4; i++) {
			this.servers[i] = new HybridServer(loader.load(new File("test", "configuration" + (i + 1) + ".xml")));
